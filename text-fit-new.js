...
function textfitNew(){
  let textfitElements = document.querySelectorAll(".textfit");

  textfitElements.forEach((el) => {
    el.style.fontSize = "initial";
    wrapInnerNew(el, '<div class="textfit-inner" style="display:inline-block;white-space:nowrap"></div>');
    el = el.querySelector(".textfit-inner");
...
function wrapInnerNew(element, dom){
  let inner = element.innerHTML;
  element.innerHTML = "";
  element.insertAdjacentHTML('beforeend', dom);
  element.firstChild.innerHTML = inner;  
}
